...
{
    // 先绘制浅色背景
    SDL_SetTextureColorModFloat(icon_->getTextureInfo().texture, 0.3f, 0.3f, 0.3f);
    game_.renderTexture(icon_->getTextureInfo(), icon_->getRenderPosition(), icon_->getSize());
    SDL_SetTextureColorModFloat(icon_->getTextureInfo().texture, 1.0f, 1.0f, 1.0f);
    // 再正常绘制
    ObjectScreen::render();
}
